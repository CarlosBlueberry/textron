<template>
    ...
    <div v-if="isLoading === false" class="business-panel">
        <div class="container p-0">
            ...
            <!-- Endx Navbar menú -->
            <hr>
            <h5 class="oExtraBold mt-5">Success Stories</h5>
            <div class="row mt-5">
                    ...
                    <table class="table text-light text-center align-middle border-dark">
                        ...
                        <tbody>
                            <tr v-for="item in filterResults" v-bind:key="item.id">
                                ...
                                <td class="p-2">{{ item.data.text }}</td>
                                <td class="p-2">
                ...
                <div class="modal-body container">
                    <div v-if="dataSelected != null">
                        ...
                        <div class="d-flex justify-content-center mt-4 mb-3">
                            <img v-if="dataSelected.data.file != 'S/D'" :src="dataSelected.data.file"
                                alt="Last success stories" class="img-fluid" width="180" />
                        </div>
...
<script>
import { success_collection_post } from '@/firebase/firebase'
...
export default {
    ...
    mounted() {
        this.success = []
        success_collection_post.get().then((r) => r.docs.map((item) => this.success.push({ id: item.id, data: item.data() })))
        setTimeout(() => {
    ...
    computed: {
        filterResults: function () {
            return this.filterProductsByName(this.filterProductsByStatus(this.success))
        }
    },
    methods: {
        filterProductsByName: function (success) {
            return success.filter(item => !item.data.text.toLowerCase().indexOf(this.searchTitle.toLowerCase()))
        },
        ...
        deleteData(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    success_collection_post.doc(id).delete().then(() => this.$mount())
                    Swal.fire(
                        'Deleted!',
        ...
        showModalData(id) {
            success_collection_post.doc(id).get().then(r => {
                this.dataSelected = {
                    id: r.id,
                    data: r.data()
                }
        ...
        updateStatus(id) {
            success_collection_post.doc(id).update({ status: this.statusUpdate }).then(() => this.$mount())
