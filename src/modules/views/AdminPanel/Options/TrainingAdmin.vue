<template>
    ...
    <div v-if="isLoading === false" class="business-panel">
        <div class="container p-0">
            ...
            <!-- Endx Navbar menú -->
            <hr>
            <h5 class="oExtraBold mt-5">Training Plan</h5>
            <div class="row mb-3">
                ...
                <div class="col-12 col-sm-12 col-md-12 col-lg-3">
                    <form action="">
                        <div class="input-group mb-3 mt-4">
                            <input v-model="searchTitle" type="text" class="form-control input-search"
                            ...
                            <tr v-for="item in filterResults" v-bind:key="item.id">
                                ...
                                <td class="p-2">
                                    <button @click="showModalData(item.id)" type="button" class="btn"
                                        ...
                                        <img v-bind:src="require('../../../../assets/images/icon-file.png')"
                                            alt="ISC user" width="30" class="img-fluid" />
                                    </button>
                                </td>
                                <td class="p-2">
                                    <div class="d-flex justify-content-center">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body container">
                    <div v-if="dataSelected != null">
                        ...
                        <hr>
                        <h5 class="text-center"><strong>Title: </strong>{{ dataSelected.data.title }}</h5>
                        <div class="d-flex justify-content-center mt-4 mb-3">
                            <img :src="dataSelected.data.img" alt="Last success stories" class="img-fluid"
                                width="180" />
                        </div>
                        ...
                        <div class="container mt-3">
                            ...
                            <p><strong>Departament: </strong>{{ dataSelected.data.departament }}</p>
                            <p><strong>Content: </strong>{{ dataSelected.data.text }}</p>
                        </div>
                        <!-- <pre>
                            {{ dataSelected }}
                        </pre> -->
...
export default {
    ...
    computed: {
        filterResults: function () {
            return this.filterProductsByStatus(this.filterDataByTitle(this.filterDataByDepartament(this.filterDataByCompany(this.filterDataByWork(this.training)))))
        }
    },
    methods: {
        filterDataByTitle: function (training) {
            return training.filter(item => !item.data.title.toLowerCase().indexOf(this.searchTitle.toLowerCase()))
        },
        ...
        filterProductsByStatus: function (success) {
            return success.filter(item => !item.data.status.indexOf(this.searchStatus))
        },
        deleteData(id) {
            Swal.fire({
                title: 'Are you sure?',
                ...
                if (result.isConfirmed) {
                    training_collection.doc(id).delete().then(() => this.$mount())
                    Swal.fire(
                        'Deleted!',
        ...
        showModalData(id) {
            training_collection.doc(id).get().then(r => {
                this.dataSelected = {
                    id: r.id,
                    data: r.data()
                }
        ...
        updateStatus(id) {
            training_collection.doc(id).update({ status: this.statusUpdate }).then(() => this.$mount())
